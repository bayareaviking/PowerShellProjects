...
Configuration CompanyServerSecure {
    ...
    Node $AllNodes.where({ $true }).NodeName {
        ...
        #LCM NEEDS KNOW WHAT CERTIFICATE TO USE
        #You may use other ways to pass the thumbprint
...
CompanyServerSecure -ConfigurationData .\demodataSecure.psd1 -OutputPath c:\dscConfigs\companyserverSecure

psedit C:\dscConfigs\companyserverSecure\SRV1.mof
psedit C:\dscConfigs\companyserverSecure\SRV2.mof
...
Invoke-Command -ScriptBlock {
 ...
 Get-Module ComputerManagementDSC,NetworkingDSC -ListAvailable
} -computername SRV1,SRV2

Set-DscLocalConfigurationManager -Path c:\dscconfigs\companyServerSecure -verbose #-ComputerName srv1,Srv2
Get-DscLocalConfigurationManager -cimsession SRV1,SRV2 | Select PSComputername,CertificateID

Clear-Host

Start-DscConfiguration -force -Wait -Verbose -Path C:\dscconfigs\companyserverSecure

Invoke-Command { Get-LocalGroupMember administrators} -comp srv1,srv2 |
...
<#
Keep security in mind with DSC
Develop your own process for working with thumbprints
Configure the LCM before pushing configurations
Up next, custom resources
#>